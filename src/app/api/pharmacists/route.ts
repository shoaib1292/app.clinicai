...
async function list(_req: NextRequest) {
  ...
  const list = await db.pharmacist.findMany({
    where: { clinicId },
...
async function create(req: NextRequest) {
  ...
  const existing = await db.pharmacist.findUnique({ where: { email: emailLower } })
  if (existing) return err('Email already in use', 409)

  const passwordHash = await hashPassword(password)
  const pharm = await db.pharmacist.create({
    data: {
  ...
  await auditLog({
    ...
    action: 'pharmacist_created', target: pharm.id,
  ...
  const clinicName = await getClinicNameForUser('pharmacist', clinicId)
  await sendStaffInvite({ id: pharm.id, name: pharm.name, email: pharm.email, userType: 'pharmacist' }, clinicName)

  return ok({ id: pharm.id, name: pharm.name, email: pharm.email, phone: pharm.phone, active: pharm.active })
}
