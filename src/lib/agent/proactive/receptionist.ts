...
export async function receptionistProactive(
  message: string,
  ctx: AgentContext,
  ...
  const bookingIntent = /appointment|book|leni|lena|booking|slot|time|waqt/i.test(lower)
  const cancelIntent = /cancel|cancel karni|cancel kar/i.test(lower)
  const statusIntent = /situation|status|kya chal raha|kaisa hai|queue|token/i.test(lower)
  const doctorIntent = /doctor|kab ayenge|kaha hain/i.test(lower)
  ...
  let mentionedService = services.find((s) => lower.includes(s.name.toLowerCase()))
  ...
  if (bookingIntent && mentionedDoctor && !isConfirm) {
    ...
    if (hasTime && ctx.patientPhone) {
      ...
      if (!isPastSlot) {
        const slot = await db.slot.findFirst({
          where: { doctorId: mentionedDoctor.id, clinicId, date: targetDate, startTime: timeStr, status: 'open' },
        })
        if (slot) {
          ...
          const bookResult = await executeTool('book_appointment', {
            doctorId: mentionedDoctor.id, slotId: slot.id, patientName,
            patientPhone: ctx.patientPhone, patientGender: 'unknown', paymentMode: 'cash',
          }, ctx)
    ...
    if (!isPastSlot2) {
      ...
      if (foundSlot?.doctor) {
        ...
        if (freshSlot) {
          ...
          const patientName = nameMatch ? nameMatch[1] : (ctx.patientName || 'Patient')
          const bookResult = await executeTool('book_appointment', {
            doctorId: foundSlot.doctor.id, slotId: freshSlot.id, patientName,
            patientPhone: ctx.patientPhone, patientGender: 'unknown', paymentMode: 'cash',
          }, ctx)
    ...
    if (!confirmDoctor) {
      ...
      if (patient) {
        ...
        if (lastAppt?.doctor) confirmDoctor = lastAppt.doctor as typeof doctors[0]
      }
      if (!confirmDoctor) {
        ...
        if (docWithSlots && docWithSlots.slots?.length) confirmDoctor = docWithSlots
    ...
    if (parsed.slots?.length > 0) {
      ...
      const bookResult = await executeTool('book_appointment', {
        doctorId: confirmDoctor.id, slotId: targetSlot.id,
        patientName: ctx.patientName || 'Patient', patientPhone: ctx.patientPhone,
        patientGender: 'unknown', paymentMode: 'cash',
      }, ctx)
